...
trait ChickenAI {
  this: ChickenEntity with Actor with Timers with ImplicitContext with DefaultTimeout =>
  ...
  def movementAiBehaviour: Receive = {
    case MoveEntity =>
  ...
  def doMove(): Unit = {
    ...
    computeMoves(posX, posY, posZ, oldPosX, oldPosZ, pathMovesNumber).onComplete {
      case Success(moves) =>
        ...
        posY = newPosition.y
        posZ = newPosition.z
        world ! EntityLook(entityId, yaw = Angle(0), pitch = Angle(0), onGround = true)
        timers.startPeriodicTimer(AiTimerKey, MoveEntity, MovementTickPeriod)
    }
  }
}
