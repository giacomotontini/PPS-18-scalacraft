...
object Entities {
  ...
  abstract class Arrow extends Entity {
  ...
  class DragonFireball extends AbstractFireball

  class Fireball extends AbstractFireball //This is the large fireball shot by ghasts.

  class SmallFireball extends AbstractFireball  //This is the fireball shot by blazes and dispensers with fire charges.

  class WitherSkull extends AbstractFireball {
  ...
  class ItemFrame extends Hanging {
  ...
  class Squid extends WaterMob

  class Dolphin extends WaterMob {
  ...
  class Cod extends AbstractFish

  class PufferFish extends AbstractFish {
    @indexType(index = 1) @boxed val puffState: Int= 0
  }

  class Salmon extends AbstractFish

  class TropicalFish extends AbstractFish {
  ...
  class ZombieHorse extends AbstractHorse

  class SkeletonHorse extends AbstractHorse

  class ChestedHorse extends AbstractHorse  {
    @indexType(index = 7) val hasChest: Boolean = false
  }

  class Donkey extends ChestedHorse

  class Llama extends ChestedHorse {
    @indexType(index = 1) @boxed val strength: Int = 0
    @indexType(index = 1) @boxed val carpetColor: Int = -1
    @indexType(index = 1) @boxed val variant: Int = 0
  }

  class Mule extends ChestedHorse
  ...
  class Ocelot extends TameableAnimal {
    @indexType(index = 1) @boxed val tpe: Int = 0;
  }
  ...
  class Parrot extends TameableAnimal {
    @indexType(index = 1) @boxed val variant: Int = 0
  }

  class Villager extends Ageable {
  ...
  class Snowman extends Golem {
    @indexType(index = 0) @byte val snowmanStatus: Int = 0x10
  }

  class Shulker extends Golem {
  ...
  class VindicationIllager extends AbstractIllager

  class SpellcasterIllager extends  AbstractIllager {
    @indexType(index = 0) @byte val speel: Int = 0
  }

  class EvocationIllager extends SpellcasterIllager

  class IllusionIllager extends SpellcasterIllager

  class Vex extends Monster {
    @indexType(index = 0) @byte val vexStatus: Int = 0
  }

  class EvocationFangs extends Entity
  ...
  class Stray extends AbstractSkeleton
  ...
  class MinecartRideable extends Minecart

  class MinecartContainer extends Minecart

  class MinecartHopper extends MinecartContainer

  class MinecartChest extends MinecartContainer

  class MinecartFurnace extends Minecart {
    @indexType(index = 7) val hasFuel: Boolean = false
  }

  class MinecartTNT extends Minecart

  class MinecartSpawner extends Minecart
  ...
  class TntPrimed extends Entity {
